...
REMOTE_EXECUTABLE="/cs/faculty/benh/260/intervals"
LOCAL_PROGRAM=$1 # e.g., "./demos/lir/test1.lir"
REMOTE_PROGRAM="test.inter.lir"  # under ~/ directory
FUNCTION=$2 # e.g., FUNCTION="test"
REMOTE_OUTPUT_FILE="inter.out.tmp.txt"

# if has $3 argument, set LOCAL_OUTPUT_FILE to $3
if [ -z "$3" ]
then
    LOCAL_OUTPUT_FILE="inter.out.tmp.txt"
else
    LOCAL_OUTPUT_FILE=$3
fi
