                ...
                (Self::Range(l, u), Self::Top) => match op {
                    ...
                    lir::ArithOp::Divide => {
                        if let Number::Integer(0) = l {
                            ...
                            // I1 ÷ [-1, 1]
                            let mut v =
                                vec![*l / NUM_MINUS_ONE, *l, *u / NUM_MINUS_ONE, *u];
                            v.sort();
            ...
            while !self.worklist.is_empty() {
                ...
                for succ_label in self.reachable_successors.get(&block.id).unwrap() {
                    ...
                    #[cfg(debug_assertions)]
                    {
                        ...
                        println!(
                            "{}",
                            self.bb2store.get(&block.id).unwrap().to_string()
                        );
                        println!();
                        println!(
                            "{}",
                            self.bb2store.get(succ_label).unwrap().to_string()
                        );
                    }
